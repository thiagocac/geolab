...
export function MedicaoPage() {
  ...
  async function salvarPrecosFn() {
    ...
    try { await salvarPrecos(escopo, escopoId, buildPrecos()); await qc.invalidateQueries({ queryKey: ['escopo-medicao', escopo] }); toast('Preços salvos.', 'success'); }
  ...
  async function calcularAuto() {
    if (!escopoId) { toast('Escolha um ' + escopo + '.', 'warning'); return; } setBusy(true);
    try { const r = await computarMedicaoAuto(escopo, escopoId, inicio, fim); setItens(r.itens); setValorItens(r.valorItens); setClientId(r.clientId); toast('Medicao calculada com os preços do catálogo (Tabela de preços).', 'success'); }
    catch (e) { toast((e as Error).message, 'error'); } finally { setBusy(false); }
  }
  ...
  return (
    <div className="space-y-5">
      <PageHeader kicker="Gestão" title="Medicao / faturamento" description="Mede os itens cobráveis por contrato, cliente ou obra no período. Preço por tipo de ensaio. Exporta Excel e PDF de pré-fatura." />
      <Card>
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <Field label="Período - início" type="date" value={inicio} onChange={(e) => setInicio(e.target.value)} />
          <Field label="Período - fim" type="date" value={fim} onChange={(e) => setFim(e.target.value)} />
        </div>
        {escopoId ? (
          <>
            <div className="mt-4 text-sm font-bold text-slate-600 dark:text-slate-300">Preços por tipo de ensaio (R$)</div>
      ...
      {escopoId ? (
        <Card>
          <div className="mb-2 text-sm font-bold text-slate-600 dark:text-slate-300">Medições fechadas</div>
          {medicoes.isLoading ? <LoadingState /> : medicoes.isError ? <ErrorState message={(medicoes.error as Error).message} /> : (medicoes.data ?? []).length === 0 ? <EmptyState /> : (
            <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead><tr className="border-b text-xs uppercase text-slate-500"><th className="py-2">Competencia</th><th>Período</th><th>Status</th><th className="text-right">Total</th><th></th></tr></thead>
