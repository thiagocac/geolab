...
import { CAMPOS_ENSAIO, CAMPOS_LAUDO, CAMPOS_RECEBIMENTO, CAMPOS_CONCRETAGEM, CAMPOS_FICHA, CAMPOS_PORTAL, initCampoState, type CampoCatalogo } from '../../lib/concreto/camposEnsaioLaudo';

type AbaKey = 'ensaio' | 'laudo' | 'recebimento' | 'concretagem' | 'ficha' | 'portal';
type ColunaKey = 'ensaio_campos' | 'laudo_campos' | 'recebimento_campos' | 'concretagem_campos' | 'ficha_campos' | 'portal_campos';
...
const ABAS: { key: AbaKey; label: string; coluna: ColunaKey; cat: CampoCatalogo[]; titulo: string; desc: string; invalidate: string[] }[] = [
  ...
  { key: 'ficha', label: 'Ficha de moldagem', coluna: 'ficha_campos', cat: CAMPOS_FICHA,
    titulo: 'Campos da ficha de moldagem (PDF)',
    desc: 'Liga/desliga campos, colunas e blocos da ficha de moldagem impressa (NBR 5739). Titulo, interessado, obra, data e as colunas-nucleo (serie, qtde de CPs, nota fiscal, horario e CP por idade) aparecem sempre.',
    invalidate: [] },
...
export function ConfigCamposPage() {
  ...
  const [state, setState] = useState<Record<AbaKey, Record<string, boolean>>>({ ensaio: {}, laudo: {}, recebimento: {}, concretagem: {}, ficha: {}, portal: {} });
  ...
  useEffect(() => {
    ...
    setState({
      ...
      ficha: initCampoState(CAMPOS_FICHA, (c?.ficha_campos ?? {}) as Record<string, unknown>),
