...
export function LaudosPage() {
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}><input className="input" aria-label="Buscar laudo" placeholder="Buscar por Nº do relatório…" value={busca} onChange={(e) => setBusca(e.target.value)} style={{ maxWidth: 280 }} /><select className="input" aria-label="Filtrar por obra" value={obraFiltro} onChange={(e) => { setObraFiltro(e.target.value); setPage(0); }} style={{ maxWidth: 240 }}><option value="">Todas as obras</option>{(worksFiltro.data ?? []).map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select><select className="input" aria-label="Filtrar por status" value={statusFiltro} onChange={(e) => { setStatusFiltro(e.target.value); setPage(0); }} style={{ maxWidth: 180 }} title="Status do laudo"><option value="">Todos os status</option><option value="pendente">A aprovar/emitir</option><option value="emitido">Emitidos</option></select></div><Button onClick={() => setNovo(true)}>Novo laudo</Button></div>
          ...
          {elegiveis.isLoading ? <LoadingState /> : (elegiveis.data ?? []).length === 0 ? <EmptyState /> : (
            <>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 700 }}>
                <input type="checkbox" aria-label="Selecionar todos os laudos" checked={sel.size > 0 && sel.size === (elegiveis.data ?? []).length} onChange={(e) => setSel(e.target.checked ? new Set((elegiveis.data ?? []).map((c) => c.id)) : new Set())} /> Selecionar todas ({(elegiveis.data ?? []).length})
              </label>
