    ...
    <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 24, background: 'var(--paper)' }}>
      <div style={{ width: '100%', maxWidth: 520, display: 'grid', gap: 16 }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 11, letterSpacing: '.16em', textTransform: 'uppercase', fontWeight: 800, color: 'var(--magenta)' }}>Consulte GEO</div>
          <div style={{ fontSize: 26, fontWeight: 800, color: 'var(--ink)' }}>GEOLAB</div>
          <div style={{ fontSize: 13, color: 'var(--ink-faint)' }}>Validacao publica de laudo</div>
        </div>
        <div className="card" style={{ padding: 20 }}>
          {!codigo ? <p style={{ margin: 0, color: 'var(--ink-faint)' }}>Codigo nao informado.</p>
            : q.isLoading ? <p style={{ margin: 0, color: 'var(--ink-faint)' }}>Verificando...</p>
            : !v?.found ? (
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--magenta)' }}>Laudo nao encontrado</div>
                <p style={{ fontSize: 13, color: 'var(--ink-faint)', margin: '6px 0 0' }}>O codigo informado nao corresponde a um laudo registrado.</p>
              </div>
            ) : (
              <div style={{ display: 'grid', gap: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <span style={{ width: 10, height: 10, borderRadius: 99, background: v.status === 'emitido' ? '#16a34a' : '#d97706' }} />
                  <strong style={{ color: 'var(--ink)' }}>{v.status === 'emitido' ? 'Laudo emitido e autentico' : 'Laudo registrado (' + v.status + ')'}</strong>
                </div>
                <Row k="Numero do laudo" val={v.numero} />
                <Row k="Concretagem" val={v.concretagem} />
                <Row k="Data de emissao" val={v.data_emissao} />
                <Row k="Revisao" val={'R' + (v.revisao ?? 0)} />
                <Row k="Laboratorio" val={v.laboratorio} />
