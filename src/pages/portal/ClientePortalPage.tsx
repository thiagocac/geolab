...
export function ClientePortalPage() {
  ...
  const concretagens = useQuery({ queryKey: ['portal-concretagens', search], queryFn: () => listPortalConcretagens(search) });
  const laudos = useQuery({ queryKey: ['portal-laudos', search], queryFn: () => listPortalLaudos(search) });
  ...
  async function enviar() {
    ...
    try {
      ...
      setRows([blank()]);
      await Promise.all([qc.invalidateQueries({ queryKey: ['portal-concretagens'] }), qc.invalidateQueries({ queryKey: ['programacoes'] })]);
  ...
  return (
    <section className="space-y-5">
      <PageHeader kicker="Portal do cliente" title="Programações, resultados e laudos" description="Solicite novas concretagens em uma grade simples e acompanhe os resultados e laudos emitidos pelo laboratório." />
      <Card>
      ...
      <Card>
        <CardHeader title="Consultar concretagens e laudos">Pesquise por número, local/peça ou traço.</CardHeader>
        <div className="p-4"><Field label="Pesquisa" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Digite parte do código, local ou número do laudo" /></div>
      </Card>

      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader title="Concretagens">Status de programações e resultados já lançados.</CardHeader>
          {concretagens.isLoading ? <LoadingState /> : concretagens.isError ? <ErrorState message={(concretagens.error as Error).message} /> : (concretagens.data ?? []).length === 0 ? <EmptyState /> : <div className="divide-y divide-slate-100 dark:divide-slate-800">{(concretagens.data ?? []).map((c) => <div key={c.id} className="p-4 text-sm"><div className="font-black text-slate-950 dark:text-slate-50">{c.codigo ?? '(sem código)'} · {c.status}</div><div className="mt-1 text-slate-500">{c.client_works?.nome ?? '-'} · {c.data_real ?? c.data_programada ?? '-'} · {c.local_texto ?? '-'}</div>{(c.lab_reports ?? []).length ? <div className="mt-2 flex flex-wrap gap-2">{(c.lab_reports ?? []).map((l) => <Button key={l.id} variant="secondary" onClick={() => void abrir(l.id)}>Baixar {l.numero}</Button>)}</div> : null}</div>)}</div>}
        </Card>
        <Card>
          <CardHeader title="Laudos disponíveis">Arquivos emitidos ou em revisão pelo laboratório.</CardHeader>
          {laudos.isLoading ? <LoadingState /> : laudos.isError ? <ErrorState message={(laudos.error as Error).message} /> : (laudos.data ?? []).length === 0 ? <EmptyState /> : <div className="divide-y divide-slate-100 dark:divide-slate-800">{(laudos.data ?? []).map((l) => <div key={l.id} className="flex flex-wrap items-center justify-between gap-3 p-4 text-sm"><div><div className="font-black text-slate-950 dark:text-slate-50">{l.numero} · {l.status}</div><div className="mt-1 text-slate-500">{l.client_works?.nome ?? '-'} · {l.data_emissao ?? '-'}</div></div><Button variant="secondary" onClick={() => void abrir(l.id)}>Baixar PDF</Button></div>)}</div>}
        </Card>
      </div>
    </section>
  );
