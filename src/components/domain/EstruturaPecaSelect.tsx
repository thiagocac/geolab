...
export function EstruturaPecaSelect({ estruturas, onPick, initialLocal }: { estruturas: Estrutura[]; onPick: (v: EstruturaPecaPick) => void; initialLocal?: string }) {
  ...
  return (
    <>
      <SelectField label="Estrutura" value={estId} onChange={(e) => { setEstId(e.target.value); setPecaId(''); }}>
        <option value="">—</option>
        {estruturas.map((e) => <option key={e.id} value={e.id}>{e.nome}</option>)}
      </SelectField>
      <SelectField label="Peça" value={pecaId} disabled={!est} onChange={(e) => { const id = e.target.value; setPecaId(id); const p = est?.pecas.find((x) => x.id === id); if (p && est) onPick({ local: est.nome + ' · ' + p.nome, estrutura_id: est.id, estrutura_nome: est.nome, peca_id: p.id, peca_nome: p.nome }); }}>
