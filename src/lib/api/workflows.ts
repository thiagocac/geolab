...
export const FLOW_CATALOG: FlowDef[] = [
  { key: 'laudo_insatisfatorio', label: 'Liberação de laudo insatisfatório', entity: 'lab_report', permission: 'laudo.aprovar', hint: 'Exemplar abaixo do fck na idade de controle exige aprovação antes de emitir o laudo (bloqueio automático da emissão entra na Leva W3).' },
  { key: 'laudo_inconsistente', label: 'Liberação de laudo inconsistente', entity: 'lab_report', permission: 'laudo.aprovar', hint: 'CP de idade menor com resultado maior que o de idade maior (mesma amostra/NF). Tolerância configurável no gatilho.' },
  { key: 'medicao_interna', label: 'Aprovação de medição', entity: 'medicao', permission: 'medicao.decidir', hint: 'Aprovação interna da medição antes do envio ao cliente (bloqueio do envio entra na Leva W4).' },
  { key: 'premiacao', label: 'Aprovação de premiação', entity: 'bonus_cycle', permission: 'premiacao.gerenciar', hint: 'Ciclo calculado passa por aprovação em etapas; ao aprovar, o ciclo é efetivado (mesmo efeito do botão atual).' },
  { key: 'portal_solicitacao', label: 'Solicitação do portal do cliente', entity: 'portal_correcao_pedido', permission: 'portal.gerenciar', hint: 'Pedidos de correção vindos do Portal do Cliente passam por aprovação interna antes de aplicar.' },
  { key: 'documento', label: 'Aprovação documental', entity: 'lab_document', permission: 'docgate.gerenciar', hint: 'Documentos do gate documental aprovados em etapas (substitui a revisão de uma pessoa quando ativo).' },
