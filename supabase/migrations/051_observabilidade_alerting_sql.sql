-- 051_observabilidade_alerting_sql.sql — Alarmes SQL (complemento da Camada 7).
-- Alvo: GEOLAB. Spec: OBSERVABILIDADE.md §VII.3. Doadores: GEOCON (telemetry_pg_alarm, v283, v291).
-- Aditiva e idempotente (CREATE OR REPLACE + cron.schedule por nome). Aplicar via MCP. NÃO testável no sandbox.
--
-- 3 avaliadores 100% Postgres, agendados direto no pg_cron (escalonados :20/:25/:35 p/ não competir):
--   telemetry_pg_alarm_run      (kind pg_query/pg_table) — queries lentas + tabelas inchadas
--   telemetry_release_alarm_run (kind release_health)    — crash-free da release mais recente
--   telemetry_email_alarm_run   (kind email_health)      — bounce%/complaint% do Resend
--
-- Adaptações vs doador:
--   * NÃO redefino resolve_telemetry_alerts — o da 049 já exclui pg_%/release_health/email_health/
--     ops_health/schedule_health/frontend_health (cada alarme resolve só o próprio kind).
--   * Colunas de settings (alert_pg_*/alert_crash_free_*/alert_email_*) já vêm na 048 — não re-altero.
--   * pg: o sinal de pg_stat_statements (schema 'extensions' no Supabase) fica num bloco EXCEPTION —
--     se a extensão não estiver habilitada, pula esse sinal sem derrubar o alarme (bloat ainda roda).
--   * release/email portam verbatim: release lê client_telemetry_log; email lê notification_dispatch_log
--     (mesma tabela/colunas do GEOLAB — resend_id/bounced_at/complained_at/status/deleted_at/created_at).
--   * Crons renomeados para 'concresoft-*'.

-- ============================================================================
-- telemetry_pg_alarm_run() — saúde do Postgres (queries lentas + bloat)
-- ============================================================================
...
BEGIN
  ...
  -- (a) Queries lentas e frequentes (pg_stat_statements). Pulado com elegância se a extensão não existir.
  BEGIN
    FOR r IN
    ...
    LOOP
      ...
      PERFORM public.raise_telemetry_alert(v_key, 'pg_query', v_sev,
        'Query lenta e frequente (média ' || r.mean_ms || ' ms)',
        r.calls || ' chamadas, média ' || r.mean_ms || ' ms (limiar ' || s.alert_pg_mean_ms ||
  ...
  EXCEPTION WHEN OTHERS THEN
    -- pg_stat_statements ausente/sem permissão → ignora o sinal de queries lentas.
    NULL;
  END;

  -- (b) Tabelas inchadas (dead tuples) em tabelas grandes
  FOR r IN
  ...
  -- (c) Resolve SÓ os pg_* que saíram de alarme
  ...
  PERFORM public.record_cron_heartbeat('telemetry-pg-alarm', 120,
    CASE WHEN v_raised > 0 THEN 'warning' ELSE 'ok' END,
    CASE WHEN v_raised > 0 THEN v_raised || ' alerta(s) de DB-health' ELSE NULL END,
    'Saúde do Postgres (queries lentas, bloat)');
  RETURN jsonb_build_object('status', CASE WHEN v_raised>0 THEN 'warning' ELSE 'ok' END,
    'raised', v_raised, 'resolved', v_resolved);
END;
$function$;
REVOKE EXECUTE ON FUNCTION public.telemetry_pg_alarm_run() FROM public;
GRANT EXECUTE ON FUNCTION public.telemetry_pg_alarm_run() TO service_role;

-- ============================================================================
-- telemetry_release_alarm_run() — crash-free da release mais recente (24h). Off por default.
-- ============================================================================
...
BEGIN
  ...
  IF cur.app_version IS NOT NULL AND cur.crash_free < s.alert_crash_free_min_pct THEN
    ...
    PERFORM public.raise_telemetry_alert(v_key, 'release_health', v_sev,
      ...
      cur.err_sessions || ' de ' || cur.sessions || ' sessões com erro nas últimas 24h (crash-free ' ||
        cur.crash_free || '%, limiar ' || s.alert_crash_free_min_pct || '%).' ||
        coalesce(' Versões anteriores: ' || prev_cf || '%.', ''),
  ...
  GET DIAGNOSTICS v_resolved = ROW_COUNT;
  PERFORM public.record_cron_heartbeat('telemetry-release-alarm', 120,
    CASE WHEN v_raised > 0 THEN 'warning' ELSE 'ok' END,
    CASE WHEN v_raised > 0 THEN v_raised || ' alerta(s) de crash-free' ELSE NULL END,
    'Crash-free por release (regressão pós-deploy)');
  RETURN jsonb_build_object('status', CASE WHEN v_raised>0 THEN 'warning' ELSE 'ok' END,
    'raised', v_raised, 'resolved', v_resolved, 'version', cur.app_version, 'crash_free', cur.crash_free);
END;
$function$;
REVOKE EXECUTE ON FUNCTION public.telemetry_release_alarm_run() FROM public;
GRANT EXECUTE ON FUNCTION public.telemetry_release_alarm_run() TO service_role;

-- ============================================================================
-- telemetry_email_alarm_run() — bounce%/complaint% do Resend (notification_dispatch_log).
-- ============================================================================
...
BEGIN
  ...
  IF v_attempted >= s.alert_email_min_sent THEN
    ...
    IF v_bounce_pct >= s.alert_email_bounce_pct THEN
      ...
      PERFORM public.raise_telemetry_alert('email_health:bounce', 'email_health', v_sev,
        ...
        v_bounced || ' bounce(s) em ' || v_attempted || ' envios nas últimas ' || s.alert_email_window_hours ||
          'h (limiar ' || s.alert_email_bounce_pct || '%). Reputação de remetente em risco.',
    ...
    IF v_complaint_pct >= s.alert_email_complaint_pct THEN
      ...
      PERFORM public.raise_telemetry_alert('email_health:complaint', 'email_health', v_sev,
        'Taxa de reclamação alta (' || v_complaint_pct || '%)',
        v_complained || ' reclamação(ões) em ' || v_attempted || ' envios nas últimas ' || s.alert_email_window_hours ||
  ...
  PERFORM public.record_cron_heartbeat('telemetry-email-alarm', 120,
    CASE WHEN v_raised > 0 THEN 'warning' ELSE 'ok' END,
    CASE WHEN v_raised > 0 THEN v_raised || ' alerta(s) de e-mail' ELSE NULL END,
...
END;
$function$;
REVOKE EXECUTE ON FUNCTION public.telemetry_email_alarm_run() FROM public;
GRANT EXECUTE ON FUNCTION public.telemetry_email_alarm_run() TO service_role;

-- ============================================================================
-- Crons (SQL direto, escalonados; idempotente por nome). Complementam a Camada 8 (052).
-- ============================================================================
SELECT cron.schedule('concresoft-telemetry-pg-alarm',      '20 * * * *', $$ SELECT public.telemetry_pg_alarm_run(); $$);
SELECT cron.schedule('concresoft-telemetry-release-alarm', '25 * * * *', $$ SELECT public.telemetry_release_alarm_run(); $$);
SELECT cron.schedule('concresoft-telemetry-email-alarm',   '35 * * * *', $$ SELECT public.telemetry_email_alarm_run(); $$);

-- FIM 051. (Aplicar na ordem 048→049→050→051→052.) Toggles em telemetry_settings:
-- alert_pg_enabled (true), alert_crash_free_enabled (FALSE por default — ligar quando houver sessões),
-- alert_email_enabled (true). pg_query exige pg_stat_statements habilitado (senão o sinal é pulado).
