# GEOLAB — SOURCE VERSION v20
CACHE_NAME: consultegeo-geolab-v20 · APP_VERSION: v20

Frontend (acumulado v2→v20): login + selecao de laboratorio + shell (v2) · Cadastros (v3) ·
...
fix disparo do evento laudo_pronto (v20).

## v20 — Fix disparo do evento laudo_pronto
- lib/api/laudo.ts: gerarLaudo com fallback — quando o header x-lab-report-id nao vem exposto via
  CORS (resp.headers.get retorna null), busca o laudo mais recente da concretagem em lab_reports
  para obter o id e disparar o notify-event (laudo_pronto). Sem mexer na EF do laudo (PDF ja sai
  OK); a correcao "de manual" futura seria expor o header no cors da EF.
